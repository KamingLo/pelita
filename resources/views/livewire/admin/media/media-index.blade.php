<div x-data="{ showDeleteModal: false, deleteId: null }" class="font-sans relative">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        @forelse($items as $item)
            <div wire:key="media-{{ $item->id }}" class="group relative aspect-[4/3] overflow-hidden rounded-3xl bg-slate-200">
                ...
                <div class="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent p-6 flex flex-col justify-end">
                    <div class="flex gap-2 mb-4 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0">
                        ...
                        <button type="button" @click="deleteId = {{ $item->id }}; showDeleteModal = true" class="px-3 py-2 bg-red-500/20 backdrop-blur-md hover:bg-red-500 text-white rounded-lg transition-colors">
                    ...
                    <div class="relative">
                        @if($item->type === 'fasilitas')
                            <span class="inline-block px-2 py-0.5 bg-blue-600 text-[7px] font-bold text-white rounded mb-2 uppercase">{{ $item->category }}</span>
                        @endif
                        <h3 class="text-white font-bold text-sm line-clamp-1 uppercase">{{ $item->title }}</h3>
    ...
    <template x-teleport="body">
        ...
        >
            <div 
                ...
                class="bg-white p-8 rounded-[2.5rem] max-w-xs w-full text-center shadow-2xl transition-all transform"
            >
                ...
                <p class="text-slate-500 text-[11px] mt-2 px-4">Data yang dihapus tidak dapat dikembalikan secara permanen.</p>
                <div class="flex flex-col gap-2 mt-6">
                    <button type="button" @click="$wire.deleteMedia(deleteId); showDeleteModal = false" class="py-3.5 bg-red-500 text-white font-bold rounded-2xl hover:bg-red-600 text-[11px] transition-all active:scale-95 shadow-lg shadow-red-100 uppercase">Ya, Hapus Sekarang</button>
                    <button type="button" @click="showDeleteModal = false" class="py-3 text-slate-400 font-bold text-[11px] uppercase">Batalkan</button>
                </div>
            </div>
        </div>
    </template>
